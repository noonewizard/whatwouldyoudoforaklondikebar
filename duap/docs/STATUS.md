...
| `duap-valuation` | REFERENCE | L5 pricing vectors; apportionment and rounding tests; ordinal stability measured and enforced by `tests/ordinal_stability.rs`; the auction red-teamed in `tests/auction_redteam.rs` | No independent review. The coefficients are parameters, not measurements, and no status raises that. Measured cardinal spread between defensible policies is 11.3× at the median, and RISK-03 (withholding a reveal lowers the auction price for free) is accepted rather than fixed. |
...
| `duap-gateway` | PROTOTYPE | Integration tests over the nine observable stages; the proof endpoints carry a separate finite budget in the shipped default | Not a production HTTP stack (ADR-0011). The proof budget uses one shared bucket absent a proxy header, so one heavy client can exhaust everyone's. |
...
| `research/valuation-sensitivity/` | EXPERIMENTAL | Runs against the real pricing engine, not a reimplementation; the headline result is positive and is enforced as a test | It varies coefficients within one functional form, so it measures sensitivity to parameters and not to approach. |
