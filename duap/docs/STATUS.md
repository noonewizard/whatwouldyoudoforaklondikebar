...
| `duap-valuation` | REFERENCE | L5 pricing vectors; apportionment and rounding tests; the ordinal stability of the multiplier set is measured and enforced by `tests/ordinal_stability.rs` | No independent review. The coefficients are parameters, not measurements, and no status raises that. Measured cardinal spread between defensible policies is 11.3× at the median. |
